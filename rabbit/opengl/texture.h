...
public:
  ...
  opengl_texture(const opengl_texture &oth)
      : width(oth.width), height(oth.height), texture(oth.texture),
        opengl_format(oth.opengl_format), format(oth.format), type(oth.type) {
  ...
  ~opengl_texture() { delete[](data); }

  int point_size() { return component_size() * components_count(); }
  ...
  void reference_buffer(unsigned char *buffer, int w, int h,
                        GLint _opengl_format, GLint _format, GLint _type) {
    invalidate();
  ...
  uint8_t &at8(int i, int j) { return ((uint8_t *)data)[i * width + j]; }
  ...
  void bind() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, opengl_format, width, height, 0, format,
                 type, data);
  ...
  void bind(GLint channel) {
    GLCHECK(glGenTextures(1, &texture));
    GLCHECK(glBindTexture(GL_TEXTURE_2D, texture));
    GLCHECK(glTexImage2D(GL_TEXTURE_2D, 0, channel, width, height, 0, channel,
                         type, data));
  ...
  void rebind() {
    GLCHECK(glBindTexture(GL_TEXTURE_2D, texture));
    GLCHECK(glTexImage2D(GL_TEXTURE_2D, 0, opengl_format, width, height, 0,
                         format, type, data));
